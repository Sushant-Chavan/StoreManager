...
bool writeConfig::writeItemData()
{
	...
	// create and open a Temporary File to copy the comments in the begining o the config file
	...
	// copy the commens from the config file to the temporary file
	do
	{
		infile.getline(filestream, BUFF_SIZE);
		tempFile << filestream;
	...
	// open the temp file as input file
	...
	// open the config file as output file
	...
	// Copy the comments from the temp file to the config file
	do
	{
		infile.getline(filestream, BUFF_SIZE);
		outFile << filestream;
	...
	// Close and delete the config file
	infile.close();
	if (remove("F:\\Projects\\StoreManager\\Debug\\Config\\Temp.txt") != 0)
		cout << "Error deleting Temp.txt!!";

	// Copy the configurations present in the item Manager in the config file
