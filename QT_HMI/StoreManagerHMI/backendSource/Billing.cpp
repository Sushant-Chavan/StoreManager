...
void bill::billedItem::display()
{
    ...
    qDebug() << myItem.getItemName().c_str() << ", " << myItem.getItemQuantity() << "gms, Rs." << myItem.getItemPrice() << " per unit." << "----> " << quantity << " units";
}
...
void bill::removeItem(unsigned int itemCode)
{
	...
	else
	{
        ...
        qDebug() << "Item Not Present In The Bill!!";
	}
}
...
float bill::compute()
{
    ...
    qDebug() << "Current Bill Amount = " << totalAmount;
